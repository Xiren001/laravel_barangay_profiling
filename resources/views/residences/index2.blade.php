...
<div class="container-div">
    ...
    <div class="table-wrapper">
            ...
            <div class="roww">
                <div style="display: flex; flex-direction:row; padding:1rem 4rem; width:100%; gap:1rem;">
                    ...
                    <div class="textt" style="width: 100%;">
                        <a href="{{ route('residences.create') }}" class="btn btn-add" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Create Certificate</span></a>
                    </div>
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $residence->tracking }}</td>
                    <td>{{ $residence->name }}</td>
                    <td>{{ $residence->address }}</td>
                    <td>{{ $residence->date }}</td>
                    <td>
                        <select name="status" class="form-control @error('status') is-invalid @enderror" id="inputStatus" data-business-id="{{ $residence->id }}" onchange="updateStatus(this)">
                            <option hidden value="{{ $residence->status }}">{{ $residence->status }}</option>
                    ...
                    <td width="200px">
                        <form action="{{ route('residences.destroy',$residence->id) }}" method="POST">

                            <a class="btn btn-info btn-sm" href="{{ route('residences.show',$residence->id) }}"><i class="fa-solid fa-list"></i></a>

                            <a class="btn btn-primary btn-sm" href="{{ route('residences.edit',$residence->id) }}"><i class="fa-solid fa-pen-to-square"></i></a>
        ...
        {!! $residences->withQueryString()->links('pagination::bootstrap-5') !!}
...
<script>
    function updateStatus(selectElement) {
        ...
        const residenceId = selectElement.getAttribute('data-business-id');
        const url = `/residences/${residenceId}`;
